...
package fpu_pkg;
    ...
    class fpu_constrained_sequence extends uvm_sequence #(fpu_transaction);
        ...
        function bit [31:0] generate_operand(operand_category_t category);
            bit sign;
            bit [7:0]  exp;
            bit [22:0] frac;
        ...
        virtual task body();
            fpu_transaction tx;
            int i;
            operand_category_t cat_a, cat_b;
            int rand_val;
    ...
    class fpu_corner_case_sequence extends uvm_sequence #(fpu_transaction);
        ...
        virtual task body();
            ...
            // +0, -0
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h00000000; tx.b = 32'h3F800000; tx.op = 0;
            start_item(tx); finish_item(tx);

            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h80000000; tx.b = 32'h3F800000; tx.op = 0;
            start_item(tx); finish_item(tx);

            // smallest subnormal / normal
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h00000001; tx.b = 32'h00800000; tx.op = 2;
            start_item(tx); finish_item(tx);

            // largest subnormal / normal
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h007FFFFF; tx.b = 32'h00800000; tx.op = 2;
            start_item(tx); finish_item(tx);

            // smallest normal / normal
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h00800000; tx.b = 32'h40800000; tx.op = 3;
            start_item(tx); finish_item(tx);

            // largest normal / normal
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h7F7FFFFF; tx.b = 32'h3F800000; tx.op = 3;
            start_item(tx); finish_item(tx);

            // max finite + max finite -> overflow
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h7F7FFFFF; tx.b = 32'h7F7FFFFF; tx.op = 0;
            start_item(tx); finish_item(tx);

            // max finite * 2.0 -> overflow
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h7F7FFFFF; tx.b = 32'h40000000; tx.op = 2;
            start_item(tx); finish_item(tx);

            // 1.0 / 3.0 -> exact RNE
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h3F800000; tx.b = 32'h40400000; tx.op = 3;
            start_item(tx); finish_item(tx);

            // 1.0 / 10.0 -> exact RNE
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h3F800000; tx.b = 32'h41200000; tx.op = 3;
            start_item(tx); finish_item(tx);

            // RNE tie cases (halfway)
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h3F800000; tx.b = 32'h34000000; tx.op = 0;
            start_item(tx); finish_item(tx);

            // Subnormal rounding to normal boundary
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h007FFFFF; tx.b = 32'h00000001; tx.op = 0;
            start_item(tx); finish_item(tx);
        endtask
    endclass

    // ------------------------------------------------------------
    // Original sequence (kept for smoke test)
    // ------------------------------------------------------------
    ...
    class fpu_driver extends uvm_driver #(fpu_transaction);
        ...
        virtual function void build_phase(uvm_phase phase);
            ...
            if (!uvm_config_db #(virtual fpu_if)::get(this, "", "vif", vif)) begin
                `uvm_fatal("NOVIF", "fpu_driver: virtual interface not set")
            end
        endfunction
    ...
    // ------------------------------------------------------------
    // Monitor – registered DUT, 1‑cycle latency
    // ------------------------------------------------------------
    class fpu_monitor extends uvm_monitor;
        ...
        virtual function void build_phase(uvm_phase phase);
            super.build_phase(phase);
            if (!uvm_config_db #(virtual fpu_if)::get(this, "", "vif", vif)) begin
                `uvm_fatal("NOVIF", "fpu_monitor: virtual interface not set")
            end
        endfunction

        virtual task run_phase(uvm_phase phase);
            fpu_transaction tx;
            bit [31:0] prev_a, prev_b;
            bit [2:0]  prev_op;
            bit have_prev = 0;
            ...
            forever begin
                ...
                if (vif.rst) begin
                    ...
                    if (have_prev) begin
                        ...
                        if (!tx.ready) begin
                            `uvm_error("MON", "Expected ready=1")
                        end
    ...
    class fpu_agent extends uvm_agent;
        ...
        virtual function void build_phase(uvm_phase phase);
            ...
            sequencer = uvm_sequencer #(fpu_transaction)::type_id::create("sequencer", this);
        endfunction
    ...
    class fpu_smoke_test extends uvm_test;
        ...
        function new(string name = "fpu_smoke_test", uvm_component parent = null);
            super.new(name, parent);
        endfunction
        ...
        virtual task run_phase(uvm_phase phase);
            ...
            while (received < 20) begin
                ...
                if (tx.result === 32'bx) begin
                    `uvm_error("SMOKE", "Transaction has X result")
                end
            end
            `uvm_info("SMOKE", $sformatf("Received %0d transactions", received), UVM_MEDIUM)
            phase.drop_objection(this);
        endtask
    endclass

    // ------------------------------------------------------------
    // Scoreboard – deterministic, objection‑controlled
    // ------------------------------------------------------------
    class fpu_scoreboard extends uvm_component;
        ...
        string script_path = "../reference/scoreboard_bridge.py";
        ...
        virtual task compare_all();
            int fd_in, fd_res, ret;
            string cmd, line_expected;
            int mismatches = 0;
            int unsupported = 0;
            ...
            fd_in = $fopen(input_file, "w");
            if (!fd_in) begin
                `uvm_error("SCRD", "Cannot open input file")
                return;
            end
            foreach (tx_queue[i]) begin
                $fwrite(fd_in, "%h %h %h\n", tx_queue[i].a, tx_queue[i].b, tx_queue[i].op);
            end
            $fclose(fd_in);

            cmd = $sformatf("python3 %s %s %s", script_path, input_file, output_file);
            `uvm_info("SCRD", $sformatf("Running: %s", cmd), UVM_MEDIUM)
            ret = $system(cmd);
            if (ret != 0) begin
                `uvm_error("SCRD", "Python bridge returned non-zero status")
                return;
            end

            fd_res = $fopen(output_file, "r");
            if (!fd_res) begin
                `uvm_error("SCRD", "Cannot open output file")
                return;
            end

            foreach (tx_queue[i]) begin
                ret = $fgets(line_expected, fd_res);
                if (ret == 0) begin
                    `uvm_error("SCRD", "Missing expected result")
                    break;
                end

                while (line_expected.len() > 0 &&
                       (line_expected[line_expected.len()-1] == "\n" ||
                        line_expected[line_expected.len()-1] == "\r")) begin
                    line_expected = line_expected.substr(0, line_expected.len()-2);
                end
                ...
                if ($sscanf(line_expected, "%h", expected_val) != 1) begin
                    `uvm_error("SCRD", $sformatf("Bad expected line: %s", line_expected))
                    continue;
                end

                if (tx_queue[i].result !== expected_val) begin
                    `uvm_error("SCRD", $sformatf("Mismatch: a=%h b=%h op=%h DUT=%h expected=%h",
                                                  tx_queue[i].a, tx_queue[i].b, tx_queue[i].op,
            ...
            `uvm_info("SCRD",
                      $sformatf("Scoreboard: total %0d, compared %0d, unsupported %0d, mismatches %0d",
                                total, total - unsupported, unsupported, mismatches),
                      UVM_LOW)
        endtask
    endclass

    // ------------------------------------------------------------
    // Functional Coverage Component (Manual)
    // ------------------------------------------------------------
    class fpu_coverage extends uvm_component;
        ...
        function string classify_operand(bit [31:0] val);
            bit [7:0] exp = val[30:23];
            bit [22:0] frac = val[22:0];
            if (exp == 0 && frac == 0)      return "ZERO";
            if (exp == 0 && frac != 0)      return "SUBNORMAL";
            if (exp == 8'hFF && frac == 0)  return "INFINITY";
            if (exp == 8'hFF && frac != 0)  return "NAN";
            return "NORMAL";
        endfunction

        function string classify_exponent_region(bit [7:0] exp);
            if (exp == 0)       return "ZERO/SUB";
            if (exp == 1)       return "MIN_NORMAL";
            if (exp == 254)     return "MAX_NORMAL";
            if (exp == 255)     return "INF/NAN";
            if (exp <= 64)      return "LOW";
            if (exp <= 192)     return "MID";
            return "HIGH";
        endfunction

        function void sample_transaction(fpu_transaction tx);
            string cat_a, cat_b, cat_res;
            ...
            sign_a_hit[tx.a[31]]++;
            sign_b_hit[tx.b[31]]++;
            region_a = classify_exponent_region(tx.a[30:23]);
            region_b = classify_exponent_region(tx.b[30:23]);
            exp_a_region_hit[region_a]++;
            exp_b_region_hit[region_b]++;
        endfunction

        virtual function void report_phase(uvm_phase phase);
            string op_names[0:3] = '{"ADD", "SUB", "MUL", "DIV"};
            `uvm_info("COVERAGE", "===== FPU FUNCTIONAL COVERAGE REPORT =====", UVM_LOW)
            `uvm_info("COVERAGE", "Operations:", UVM_LOW)
            foreach (op_names[i])
                `uvm_info("COVERAGE", $sformatf("  %4s : %0d", op_names[i], op_hit[i]), UVM_LOW)
            `uvm_info("COVERAGE", "Operand A categories:", UVM_LOW)
            foreach (a_category_hit[cat])
                `uvm_info("COVERAGE", $sformatf("  %10s : %0d", cat, a_category_hit[cat]), UVM_LOW)
            `uvm_info("COVERAGE", "Operand B categories:", UVM_LOW)
            foreach (b_category_hit[cat])
                `uvm_info("COVERAGE", $sformatf("  %10s : %0d", cat, b_category_hit[cat]), UVM_LOW)
            `uvm_info("COVERAGE", "Result categories:", UVM_LOW)
            foreach (result_category_hit[cat])
                `uvm_info("COVERAGE", $sformatf("  %10s : %0d", cat, result_category_hit[cat]), UVM_LOW)
            `uvm_info("COVERAGE", $sformatf("Sign A: pos=%0d neg=%0d", sign_a_hit[0], sign_a_hit[1]), UVM_LOW)
            `uvm_info("COVERAGE", $sformatf("Sign B: pos=%0d neg=%0d", sign_b_hit[0], sign_b_hit[1]), UVM_LOW)
            `uvm_info("COVERAGE", "Exponent regions for A:", UVM_LOW)
            foreach (exp_a_region_hit[region])
                `uvm_info("COVERAGE", $sformatf("  %10s : %0d", region, exp_a_region_hit[region]), UVM_LOW)
            `uvm_info("COVERAGE", "Exponent regions for B:", UVM_LOW)
            foreach (exp_b_region_hit[region])
                `uvm_info("COVERAGE", $sformatf("  %10s : %0d", region, exp_b_region_hit[region]), UVM_LOW)
            `uvm_info("COVERAGE", "=============================================", UVM_LOW)
        endfunction
    endclass

    // ------------------------------------------------------------
    // Regression Test with scoreboard and coverage (1016 vectors)
    // ------------------------------------------------------------
    class fpu_regression_test extends uvm_test;
        ...
        function new(string name = "fpu_regression_test", uvm_component parent = null);
            super.new(name, parent);
        endfunction
        ...
        virtual function void connect_phase(uvm_phase phase);
            super.connect_phase(phase);
            agent.monitor.ap.connect(scoreboard.mon_fifo.analysis_export);
            agent.monitor.ap.connect(coverage.cov_fifo.analysis_export);
        endfunction
        ...
        virtual task run_phase(uvm_phase phase);
            ...
            phase.raise_objection(this);
            #25;
            seq = fpu_constrained_sequence::type_id::create("seq");
    ...
    // ------------------------------------------------------------
    // Corner-Case Test with scoreboard and coverage
    // ------------------------------------------------------------
    class fpu_corner_case_test extends uvm_test;
        ...
        function new(string name = "fpu_corner_case_test", uvm_component parent = null);
            super.new(name, parent);
        endfunction
        ...
        virtual task run_phase(uvm_phase phase);
            ...
            phase.raise_objection(this);
            #25;
            seq = fpu_corner_case_sequence::type_id::create("seq");
            seq.start(agent.sequencer);
            phase.drop_objection(this);
        endtask
    endclass
    ...
    // ------------------------------------------------------------
    // Long Regression Test (10,000 random + 16 targeted)
    // ------------------------------------------------------------
    class fpu_long_regression_test extends uvm_test;
        ...
        function new(string name = "fpu_long_regression_test", uvm_component parent = null);
            super.new(name, parent);
        endfunction
    ...
    class fpu_closure_sequence extends uvm_sequence #(fpu_transaction);
        ...
        function new(string name = "fpu_closure_sequence");
            super.new(name);
        endfunction
        ...
        virtual task body();
            ...
            // 1) fp_mul: provoke rounded_sig_ext[24] overflow
            //    a = 0x3F7FFFFE, b = 0x3F800001, op = MUL
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h3F7FFFFE;
            tx.b = 32'h3F800001;
            tx.op = 2;
            ...
            // 3) fp_div subnormal sticky accumulation
            tx = fpu_transaction::type_id::create("tx");
            tx.a = 32'h00000001;
            tx.b = 32'h7F7FFFFF;
            tx.op = 3;
    ...
    class fpu_closure_test extends uvm_test;
        ...
        function new(string name = "fpu_closure_test", uvm_component parent = null);
            super.new(name, parent);
        endfunction
        ...
        virtual task run_phase(uvm_phase phase);
            ...
            phase.raise_objection(this);
            #25;
            seq = fpu_closure_sequence::type_id::create("seq");
            seq.start(agent.sequencer);
            phase.drop_objection(this);
        endtask
    endclass
